...
#find the percentage of incorrect variants in each bin from the clinvar_wg dataset for BayesDel. This is the incorrect prediction tolerance. Also calculate the number of variants needed in each interval to be concordant or discordant using the binomial theorem
  
strong_pathogenic_percentage_bd = path_incorrect(filter(clinvar_wg, clinvar_wg$BayesDel_nsfp33a_noAF >= 0.5))
...
moderate_pathogenic_percentage_bd = path_incorrect(filter(clinvar_wg, clinvar_wg$BayesDel_nsfp33a_noAF < 0.5 & clinvar_wg$BayesDel_nsfp33a_noAF >= 0.27))
...
supporting_pathogenic_percentage_bd = path_incorrect(filter(clinvar_wg, clinvar_wg$BayesDel_nsfp33a_noAF < 0.27 & clinvar_wg$BayesDel_nsfp33a_noAF >= 0.13))
...
moderate_benign_percentage_bd = ben_incorrect(filter(clinvar_wg, clinvar_wg$BayesDel_nsfp33a_noAF <= -0.36))
...
supporting_benign_percentage_bd = ben_incorrect(filter(clinvar_wg, clinvar_wg$BayesDel_nsfp33a_noAF > -0.36 & clinvar_wg$BayesDel_nsfp33a_noAF <= -0.18))
...
conc_disc_path = function(df,bin) {
  ...
  df$incorrect = nrow(filter(df, df$Significance == 'Benign'| df$Significance == 'Likely_benign' | df$Significance == 'Benign/Likely_benign'))
  return(df)
}
...
conc_disc_ben = function(df,bin) {
  ...
  df$conc_disc = conc_disc
  df$control_variants = nrow(df)
  df$incorrect = nrow(filter(df, df$Significance == 'Pathogenic'| df$Significance == 'Likely_pathogenic' | df$Significance == 'Pathogenic/Likely_pathogenic'))
  return(df)
}
...
strong_pathogenic_bd = strong_pathogenic_bd %>% group_by(Gene) %>% group_modify(~conc_disc_path(.,strong_pathogenic_percentage_bd))
colnames(strong_pathogenic_bd)[18] = 'conc_disc_strong_pathogenic'
colnames(strong_pathogenic_bd)[19] = 'control_variants_strong_pathogenic'
colnames(strong_pathogenic_bd)[20] = 'incorrect_variants_strong_pathogenic'

#moderate pathogenic
moderate_pathogenic_bd = filter(clinvar23, clinvar23$BayesDel_noAF_score < 0.5 & clinvar23$BayesDel_noAF_score >= 0.27)
moderate_pathogenic_bd = moderate_pathogenic_bd %>% group_by(Gene) %>% group_modify(~conc_disc_path(.,moderate_pathogenic_percentage_bd))
colnames(moderate_pathogenic_bd)[18] = 'conc_disc_moderate_pathogenic'
colnames(moderate_pathogenic_bd)[19] = 'control_variants_moderate_pathogenic'
colnames(moderate_pathogenic_bd)[20] = 'incorrect_variants_moderate_pathogenic'
...
supporting_pathogenic_bd = supporting_pathogenic_bd %>% group_by(Gene) %>% group_modify(~conc_disc_path(.,supporting_pathogenic_percentage_bd))
colnames(supporting_pathogenic_bd)[18] = 'conc_disc_supporting_pathogenic'
colnames(supporting_pathogenic_bd)[19] = 'control_variants_supporting_pathogenic'
colnames(supporting_pathogenic_bd)[20] = 'incorrect_variants_supporting_pathogenic'

#moderate benign

moderate_benign_bd = filter(clinvar23,clinvar23$BayesDel_noAF_score <= -0.36)
moderate_benign_bd = moderate_benign_bd %>% group_by(Gene) %>% group_modify((~conc_disc_ben(.,moderate_benign_percentage_bd)))
colnames(moderate_benign_bd)[18] = 'conc_disc_moderate_benign'
colnames(moderate_benign_bd)[19] = 'control_variants_moderate_benign'
colnames(moderate_benign_bd)[20] = 'incorrect_variants_moderate_benign'

#supporting benign

supporting_benign_bd = filter(clinvar23, clinvar23$BayesDel_noAF_score > -0.36 & clinvar23$BayesDel_noAF_score <= -0.18)
supporting_benign_bd = supporting_benign_bd %>% group_by(Gene) %>% group_modify((~conc_disc_ben(.,supporting_benign_percentage_bd)))
colnames(supporting_benign_bd)[18] = 'conc_disc_supporting_benign'
colnames(supporting_benign_bd)[19] = 'control_variants_supporting_benign'
colnames(supporting_benign_bd)[20] = 'incorrect_variants_supporting_benign'
...
strong_pathogenic_bd$pr_variant_pass_strong_path = ifelse(strong_pathogenic_bd$conc_disc_strong_pathogenic == 'Concordant' & strong_pathogenic_bd$control_variants_strong_pathogenic >= strong_path_x_pass_bd, 'Concordant', ifelse(strong_pathogenic_bd$conc_disc_strong_pathogenic == 'Discordant' & strong_pathogenic_bd$control_variants_strong_pathogenic >= strong_path_x_fail_bd,'Discordant','Insufficient variants'))

moderate_pathogenic_bd$pr_variant_pass_moderate_path = ifelse(moderate_pathogenic_bd$conc_disc_moderate_pathogenic == 'Concordant' & moderate_pathogenic_bd$control_variants_moderate_pathogenic >= moderate_path_x_pass_bd, 'Concordant', ifelse(moderate_pathogenic_bd$conc_disc_moderate_pathogenic == 'Discordant' & moderate_pathogenic_bd$control_variants_moderate_pathogenic >= moderate_path_x_fail_bd,'Discordant','Insufficient variants'))

supporting_pathogenic_bd$pr_variant_pass_supporting_path = ifelse(supporting_pathogenic_bd$conc_disc_supporting_pathogenic == 'Concordant' & supporting_pathogenic_bd$control_variants_supporting_pathogenic >= supporting_path_x_pass_bd, 'Concordant', ifelse(supporting_pathogenic_bd$conc_disc_supporting_pathogenic == 'Discordant' & supporting_pathogenic_bd$control_variants_supporting_pathogenic >= supporting_path_x_fail_bd,'Discordant','Insufficient variants'))

supporting_benign_bd$pr_variant_pass_supporting_ben = ifelse(supporting_benign_bd$conc_disc_supporting_benign == 'Concordant' & supporting_benign_bd$control_variants_supporting_benign >= supporting_ben_x_pass_bd, 'Concordant', ifelse(supporting_benign_bd$conc_disc_supporting_benign == 'Discordant' & supporting_benign_bd$control_variants_supporting_benign >= supporting_ben_x_fail_bd,'Discordant','Insufficient variants'))

moderate_benign_bd$pr_variant_pass_moderate_ben = ifelse(moderate_benign_bd$conc_disc_moderate_benign == 'Concordant' & moderate_benign_bd$control_variants_moderate_benign >= moderate_ben_x_pass_bd, 'Concordant', ifelse(moderate_benign_bd$conc_disc_moderate_benign == 'Discordant' & moderate_benign_bd$control_variants_moderate_benign >= moderate_ben_x_fail_bd,'Discordant','Insufficient variants'))
...
wg_bd = subset(final_wg_bd, select = c(Gene, Chr, GeneID,conc_disc_strong_pathogenic,conc_disc_moderate_pathogenic,conc_disc_supporting_pathogenic,conc_disc_supporting_benign,conc_disc_moderate_benign, control_variants_strong_pathogenic, control_variants_moderate_pathogenic, control_variants_supporting_pathogenic,control_variants_moderate_benign, control_variants_supporting_benign,incorrect_variants_strong_pathogenic, incorrect_variants_moderate_pathogenic, incorrect_variants_supporting_pathogenic,incorrect_variants_moderate_benign, incorrect_variants_supporting_benign,pr_variant_pass_strong_path,pr_variant_pass_moderate_path,pr_variant_pass_supporting_path,pr_variant_pass_moderate_ben, pr_variant_pass_supporting_ben))
...
summary_1_bd = data.frame(table(wg_bd$conc_disc_strong_pathogenic))
summary_1_bd$Condition = 'Strong Pathogenic'

summary_2_bd = data.frame(table(wg_bd$conc_disc_moderate_pathogenic))
summary_2_bd$Condition = 'Moderate Pathogenic'

summary_3_bd = data.frame(table(wg_bd$conc_disc_supporting_pathogenic))
summary_3_bd$Condition = 'Supporting Pathogenic'

summary_6_bd = data.frame(table(wg_bd$conc_disc_moderate_benign))
summary_6_bd$Condition = 'Moderate Benign'

summary_7_bd = data.frame(table(wg_bd$conc_disc_supporting_benign))
...
strong_pathogenic_pr_pass_bd = filter(wg_bd, wg_bd$control_variants_strong_pathogenic >= strong_path_x_pass_bd & wg_bd$conc_disc_strong_pathogenic == 'Concordant')
strong_pathogenic_pr_fail_bd = filter(wg_bd, wg_bd$control_variants_strong_pathogenic >= strong_path_x_fail_bd  & wg_bd$conc_disc_strong_pathogenic == 'Discordant')
...
moderate_pathogenic_pr_pass_bd = filter(wg_bd, wg_bd$control_variants_moderate_pathogenic >= moderate_path_x_pass_bd & wg_bd$conc_disc_moderate_pathogenic == 'Concordant')
moderate_pathogenic_pr_fail_bd = filter(wg_bd, wg_bd$control_variants_moderate_pathogenic >= moderate_path_x_fail_bd & wg_bd$conc_disc_moderate_pathogenic == 'Discordant')
...
supporting_pathogenic_pr_pass_bd = filter(wg_bd, wg_bd$control_variants_supporting_pathogenic >= supporting_path_x_pass_bd & wg_bd$conc_disc_supporting_pathogenic == 'Concordant')
supporting_pathogenic_pr_fail_bd = filter(wg_bd, wg_bd$control_variants_supporting_pathogenic >= supporting_path_x_fail_bd & wg_bd$conc_disc_supporting_pathogenic == 'Discordant')
...
moderate_benign_pr_pass_bd = filter(wg_bd, wg_bd$control_variants_moderate_benign >= moderate_ben_x_pass_bd & wg_bd$conc_disc_moderate_benign == 'Concordant')
moderate_benign_pr_fail_bd = filter(wg_bd, wg_bd$control_variants_moderate_benign >= moderate_ben_x_fail_bd & wg_bd$conc_disc_moderate_benign == 'Discordant')
...
supporting_benign_pr_pass_bd = filter(wg_bd, wg_bd$control_variants_supporting_benign >= supporting_ben_x_pass_bd & wg_bd$conc_disc_supporting_benign == 'Concordant')
supporting_benign_pr_fail_bd = filter(wg_bd, wg_bd$control_variants_supporting_benign >= supporting_ben_x_fail_bd & wg_bd$conc_disc_supporting_benign == 'Discordant')
...
cc_bd = VUS_merge_x_bd %>% dplyr::mutate(calc_bd(.))
